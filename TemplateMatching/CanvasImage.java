...
class CanvasImage extends Canvas {
	...
	public CanvasImage(BufferedImage input) {
		image = input;
		setPreferredSize(new Dimension(image.getWidth()+border*2, image.getHeight()+border*2));
	...
	public void getPixelCoordinate(Point p) {
		p.x -= (getWidth() - image.getWidth()) / 2;
		p.y -= (getHeight() - image.getHeight()) / 2;
	}
